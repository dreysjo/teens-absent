...
class _WithKeys:
    ...
    def keys(self) -> RMKeyView:
        """Return an iterable views which yields the string keys that would
        ...
        The views also can be tested for key containment using the Python
        ``in`` operator, which will test both for the string keys represented
        in the views, as well as for alternate keys such as column objects.

        .. versionchanged:: 1.4 a key views object is returned rather than a
           plain list.
