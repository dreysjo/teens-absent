...
class SequenceView(Sequence):
    """Return a read-only views of the sequence object *target*.

    :class:`SequenceView` objects are analogous to Python's built-in
    "dictionary views" types. They provide a dynamic views of a sequence's items,
    meaning that when the sequence updates, so does the views.

        >>> seq = ['0', '1', '2']
        >>> views = SequenceView(seq)
        >>> views
        SequenceView(['0', '1', '2'])
        >>> seq.append('3')
        >>> views
...
class seekable:
    ...
    You may views the contents of the cache with the :meth:`elements` method.
    That returns a :class:`SequenceView`, a views that updates automatically:
