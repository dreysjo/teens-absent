...
def _astype_nansafe(
    ...
    copy : bool, default True
        If False, a views will be attempted but may fail, if
    ...
    if copy or arr.dtype == object or dtype == object:
        # Explicit copy, or required since NumPy can't views from / to object.
...
def astype_is_view(dtype: DtypeObj, new_dtype: DtypeObj) -> bool:
    ...
    Returns
    -------
    True if new data is a views or not guaranteed to be a copy, False otherwise
    """
    ...
    if dtype == new_dtype:
        ...
        # Potentially! a views when converting from object to string
        return True
    ...
    if numpy_dtype is not None and new_numpy_dtype is not None:
        ...
        # they are only a views when the numpy dtypes are equal, e.g.
        # int64 -> Int64 or int64[pyarrow]
        # int64 -> Int32 copies
        return numpy_dtype == new_numpy_dtype

    # Assume this is a views since we don't know for sure if a copy was made
    return True
