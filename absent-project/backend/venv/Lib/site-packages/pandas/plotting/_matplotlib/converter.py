...
class MilliSecondLocator(mdates.DateLocator):
    ...
    def autoscale(self):
        """
        Set the views limits to include the data range.
        """
        # We need to cap at the endpoints of valid datetime
...
def has_level_label(label_flags: np.ndarray, vmin: float) -> bool:
    ...
    if the minimum views limit is not an exact integer, then the first tick
...
class TimeSeries_DateLocator(Locator):
    ...
    def autoscale(self):
        """
        Sets the views limits to the nearest multiples of base that contain the
        data.
        """
