...
@pytest.mark.parametrize(
    ...
    ids=["identical", "views", "copy", "values"],
)
...
def test_to_frame(using_copy_on_write):
    ...
    # currently this always returns a "views"
    ...
    if using_copy_on_write:
        ...
        # but currently select_dtypes() actually returns a views -> mutates parent
...
def test_chained_methods(request, method, idx, using_copy_on_write):
    ...
    # when not using CoW, only the copy() variant actually gives a views
...
def test_get(using_copy_on_write, key):
    ...
    if using_copy_on_write:
        ...
        # is a views or copy or triggers a warning or not
...
def test_update_frame(using_copy_on_write):
    ...
    if using_copy_on_write:
        # df1 is updated, but its views not
...
def test_update_series(using_copy_on_write):
    ...
    if using_copy_on_write:
        # ser1 is updated, but its views not
