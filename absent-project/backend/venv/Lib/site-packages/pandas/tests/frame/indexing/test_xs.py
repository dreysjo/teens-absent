...
class TestXS:
    def test_xs(self, float_frame, datetime_frame, using_copy_on_write):
        ...
        # views is returned if possible
        series = float_frame.xs("A", axis=1)
        series[:] = 5
        if using_copy_on_write:
            # but with CoW the views shouldn't propagate mutations
    ...
    def test_xs_view(self, using_array_manager, using_copy_on_write):
        # in 0.14 this will return a views if possible a copy otherwise, but
        ...
        if using_copy_on_write:
            ...
            # INFO(ArrayManager) with ArrayManager getting a row as a views is
            # not possible
...
class TestXSWithMultiIndex:
    ...
    def test_xs_setting_with_copy_error(
        ...
        if using_copy_on_write:
            ...
            # as we are trying to write a views
