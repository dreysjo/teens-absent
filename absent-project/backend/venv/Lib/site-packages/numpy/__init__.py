...
Use the built-in ``help`` function to views a function's docstring::
...
The native Python help() does not know how to views their help, but our
...
down the list.  To views the docstring for a function, use
``np.cos?<ENTER>`` (to views the docstring) and ``np.cos??<ENTER>`` (to views
the source code).
