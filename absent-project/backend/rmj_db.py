...
def login_admin(input_nama_admin, input_password):
    with app.app_context():
        admin = db.session.query(Admin).filter_by(nama_admin=input_nama_admin , password=input_password).first()
        if admin != None:
...
def get_absen_by_tanggal(tgl):
    with app.app_context():
        abs = db.session.query(Absen.id_jemaat, Absen.waktu_absen).filter(Absen.waktu_absen == tgl).all()
        list_absen = []
