...
def test_subset_loc_rows_columns(
    ...
    # of those could actually return a views / need CoW (so this test is not
...
def test_subset_chained_getitem(
    ...
    # and whether we are slicing the columns -> in that case we have a views
...
def test_column_as_series_no_item_cache(
    ...
    # the views) should always give a new object (i.e. not make use of a cache)
    dtype_backend, DataFrame, _ = backend
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [0.1, 0.2, 0.3]})
    df_orig = df.copy()
