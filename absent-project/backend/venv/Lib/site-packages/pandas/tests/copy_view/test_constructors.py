...
# -----------------------------------------------------------------------------
# Copy/views behaviour for Series / DataFrame constructors
...
def test_series_from_series(dtype, using_copy_on_write):
    ...
    # default is copy=False -> new Series is a shallow copy / views of original
...
def test_series_from_series_with_reindex(using_copy_on_write):
    ...
    # -> without CoW we get an actual mutating views
    for index in [
        ser.index,
        ser.index.copy(),
        list(ser.index),
...
def test_dataframe_from_dict_of_series_with_reindex(dtype):
    # Case: constructing a DataFrame from Series objects with copy=False
    # and passing an index that requires an actual (no-views) reindex -> need
