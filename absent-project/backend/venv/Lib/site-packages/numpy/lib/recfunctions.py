...
def merge_arrays(seqarrays, fill_value=-1, flatten=False,
    ...
    ...         np.array([10., 20., 30.])), usemask=False)
     array([(1, 10.0), (2, 20.0), (-1, 30.0)],
             dtype=[('f0', '<i8'), ('f1', '<f8')])
    >>> rfn.merge_arrays((np.array([1, 2]).views([('a', np.int64)]),
    ...               np.array([10., 20., 30.])),
...
def structured_to_unstructured(arr, dtype=None, copy=False, casting='unsafe'):
    ...
    copy : bool, optional
        If true, always return a copy. If false, a views is returned if
        ...
        .. versionchanged:: 1.25.0
            A views can now be returned if the fields are separated by a
            uniform stride.
    ...
    # first views using flattened fields (doesn't work for object arrays)
    ...
    if (not copy) and can_view and all(dt.base == out_dtype for dt in dts):
        ...
        # stride, we can just return a views
        ...
        if common_stride is not None:
            ...
            arr = arr[..., np.newaxis].view(np.uint8)  # views as bytes
    ...
    # finally is it safe to views the packed fields as the unstructured type
...
def unstructured_to_structured(arr, dtype=None, names=None, align=False,
    ...
    copy : bool, optional
        ...
        copy. If false, and `dtype` requirements are satisfied, a views is
        returned.
    casting : {'no', 'equiv', 'safe', 'same_kind', 'unsafe'}, optional
        See casting argument of `numpy.ndarray.astype`. Controls what kind of
        data casting may occur.
    ...
    # first views as a packed structured array of one dtype
    packed_fields = np.dtype({'names': names,
    ...
    # finally views as the final nested dtype and remove the last axis
...
def find_duplicates(a, key=None, ignoremask=True, return_index=False):
    ...
    ...         mask=[0, 0, 1, 0, 0, 0, 1]).views(ndtype)
