...
class TestCasting:
            ...
            for from_dt in [from_Dt(), from_Dt().newbyteorder()]:
                ...
                for to_dt in [to_Dt(), to_Dt().newbyteorder()]:
                    ...
                    if view_off is not None:
                        # If a views is acceptable, this is "no" casting
    ...
    def test_structured_field_offsets(self, to_dt, expected_off):
        # This checks the cast-safety and views offset for swapped and "shifted"
    ...
    @pytest.mark.parametrize(("from_dt", "to_dt", "expected_off"), [
            ...
            ("i4", "V10", None),  # void is larger (no views)
            ("O", "V4", None),  # currently reject objects for views here.
            ("O", "V8", None),  # currently reject objects for views here.
            ("V4", "V3", 0),
            ("V4", "V4", 0),
            ("V3", "V4", None),
            # Note that currently void-to-other cast goes via byte-strings
            # and is not a "views" based cast like the opposite direction:
            ("V4", "i4", None),
